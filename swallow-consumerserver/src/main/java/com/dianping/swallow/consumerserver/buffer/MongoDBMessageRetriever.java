...
public class MongoDBMessageRetriever implements MessageRetriever {
    ...
    public List retriveMessage(String topicName, Long messageId, MessageFilter messageFilter) {
        ...
        if (maxIdAndMessages != null && maxIdAndMessages.size() > 0) {
            ...
            if (messageFilter != null && messageFilter.getType() == FilterType.InSet
                    && messageFilter.getParam() != null && !messageFilter.getParam().isEmpty()) {
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @Override
    public List retriveBackupMessage(String topicName, String consumerId, Long messageId) {
        List maxIdAndMessages = messageDAO.getBackupMessagesGreaterThan(topicName, consumerId, messageId, fetchSize);
