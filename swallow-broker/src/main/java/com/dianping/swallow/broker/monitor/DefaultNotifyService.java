 ...
 * with dianping.com.
 */
package com.dianping.swallow.broker.monitor;
...
import com.dianping.swallow.broker.util.IPUtils;
...
public class DefaultNotifyService implements NotifyService {
    ...
    private static final String KEY_SMS_TO          = "swallow.broker.notify.smsTo";
    ...
    public void report(String title, Map<String, Map<String, String>> msg) {
        log.info("[swallow-broker Report]" + msg);
        if (!devMode) {
            StringBuilder body = new StringBuilder();
            body.append("<strong>swallow-broker Status Report</strong><br/>");
            for (Map.Entry<String, Map<String, String>> entry : msg.entrySet()) {
                ...
                body.append("<tr align=\"center\" colspan=\"100%\"><td><font color=\"00005f\" size=\"3\"><b>")
                        .append(entry.getKey()).append("</b></font></td></tr>");
                body.append("<tr bgcolor=\"ffd588\"><th>name</th><th>value</th></tr>");
                int row = 1;
                for (Map.Entry<String, String> suEntry : entry.getValue().entrySet()) {
                    if (row++ % 2 == 0) {
                        body.append("<tr bgcolor=\"e2e2e2\"><td>").append(suEntry.getKey()).append("</td><td>")
                                .append(suEntry.getValue()).append("</td></tr>");
                    } else {
