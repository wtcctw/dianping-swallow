...
module.controller('ApiTaskListController', function($scope, $resource, $http) {
	$scope.delTaskId = -1;
	$scope.newTask = {};
	...
	$scope.removeTask = function() {
		...
		$http({
			...
			url : window.contextpath + '/console/apideploy/task/del'
		}).success(
				function(data, status, headers, config) {
					if (data.errorCode == 0) {
						window.location = window.contextpath + "/console/apideploy";
					} else {
	...
	$scope.addTask = function() {
		...
		$http({
			method : 'POST',
			data : $scope.newTask,
			url : window.contextpath + '/console/apideploy/task/add'
		}).success(
				function(data, status, headers, config) {
					if (data.errorCode == 0) {
